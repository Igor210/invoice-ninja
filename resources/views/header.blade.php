...
<nav class="navbar navbar-default navbar-fixed-top" role="navigation" style="height:60px;">
    <a @if(trans('texts.direction') != 'rtl') id="right-menu-toggle" @else id="ctrl-left-menu" style="margin:20px 0px 25px 11px;" @endif class="menu-toggle hide-phone @if(trans('texts.direction')=='rtl') pull-left @else pull-right @endif" title="{{ trans('texts.toggle_history') }}" style="cursor:pointer">
      <div class="fa fa-bars"></div>
    </a>

    <div class="navbar-header" @if(trans('texts.direction') == "rtl") style="float:right" @endif>
      ...
      <a href="#" @if(trans('texts.direction') != 'rtl') id="left-menu-toggle" @else id="ctrl-right-menu" @endif class="menu-toggle" title="{{ trans('texts.toggle_navigation') }}">
          <div class="navbar-brand">
              @if(trans('texts.direction') !== "rtl")
                @component('components/header/leftToggleBtn') @endcomponent
              @endif
    ...
    @if(trans('texts.direction') == 'rtl')
      <button id="right-menu-toggle" style="display:none"></button>
      <button id="left-menu-toggle" style="display:none"></button>
      <script>
        $("#ctrl-right-menu").click(function(e) {
            $('#right-menu-toggle').trigger('click');
        });
        $("#ctrl-left-menu").click(function(e) {
    ...
    <div class="collapse navbar-collapse" id="navbar-collapse-1">
      ...
      <div class="navbar-form @if(trans('texts.direction') == 'rtl') navbar-left @else navbar-right @endif">
      ...
      @if(trans('texts.direction') == 'rtl') @php($formcss = 'navbar-form navbar-left') @else @php($formcss = 'navbar-form navbar-right') @endif
      {!! Former::open('/handle_command')->id('search-form')->addClass($formcss)->role('search') !!}
      ...
      <ul class="nav navbar-nav   @if(trans('texts.direction') == 'rtl') navbar-left @else navbar-right @endif">
        <li class="dropdown">
...
<div id="wrapper" class='{{ session(SESSION_LEFT_SIDEBAR) ? 'toggled-left' : '' }} {{ session(SESSION_RIGHT_SIDEBAR, true) ? 'toggled-right' : '' }}'>
    ...
    <!-- Sidebar -->
    <div @if(trans('texts.direction') != 'rtl') id="left-sidebar-wrapper" @else id="right-sidebar-wrapper" style="overflow-y:hidden" @endif class="hide-phone">
    ...
    <div @if(trans('texts.direction') != 'rtl') id="right-sidebar-wrapper" style="overflow-y:hidden" @else id="left-sidebar-wrapper" @endif class="hide-phone">
        <ul class="sidebar-nav {{ Auth::user()->dark_mode ? 'sidebar-nav-dark' : 'sidebar-nav-light' }}">
